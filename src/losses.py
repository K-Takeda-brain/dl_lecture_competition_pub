...
class CLIPLoss(nn.Module):
    ...
    def forward(self, meg_embeddings, image_embeddings):
        # 画像とテキストの埋め込みを正規化
        meg_embeddings = meg_embeddings / meg_embeddings.norm(dim=-1, keepdim=True)
        image_embeddings = image_embeddings / image_embeddings.norm(dim=-1, keepdim=True)

        # ロジットの計算
        logit_scale = self.logit_scale.exp()
        logits_per_meg = logit_scale * meg_embeddings @ image_embeddings.t()
        logits_per_image = logits_per_meg.t()

        # ターゲット
        batch_size = meg_embeddings.shape[0]
        ground_truth = torch.arange(batch_size, dtype=torch.long, device=meg_embeddings.device)

        # 損失の計算
        loss = (nn.CrossEntropyLoss()(logits_per_meg, ground_truth) + nn.CrossEntropyLoss()(logits_per_image, ground_truth)) / 2
        return loss
    
    def get_probabilities(self, meg_embeddings, image_embeddings):
        # 画像とテキストの埋め込みを正規化
        meg_embeddings = meg_embeddings / meg_embeddings.norm(dim=-1, keepdim=True)
        image_embeddings = image_embeddings / image_embeddings.norm(dim=-1, keepdim=True)
